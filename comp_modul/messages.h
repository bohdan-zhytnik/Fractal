...
typedef struct {
   double length;
} msg_set_compute_BURST;

typedef struct {
   uint8_t *iters; 
} msg_compute_data_done_BURST;
...
typedef struct {
   ...
   double re;    // start of the x-coords (real)
   double im;
   double length; // number of pixels in the data message
...
typedef struct {
   ...
   union {
      ...
      msg_compute_data_burst comp_BURST;
      msg_set_compute_BURST set_comp_BURST;
      msg_compute_data_done_BURST compute_data_BURST;
   } data;
